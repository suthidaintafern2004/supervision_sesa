...
if ($mode === 'normal') {
    ...
    // ดึงข้อมูลการนิเทศ และสถานะการประเมิน
    $sql_session = "SELECT 
                        ss.supervision_date,
                        ss.subject_name,
...
// --------------------------------------------------------
    ...
    // ใช้คีย์: t_id + p_id + supervision_date
    $t_id = ($is_post ? $_POST['t_id'] : $_GET['t_id']) ?? null;
    $p_id = ($is_post ? $_POST['p_id'] : $_GET['p_id']) ?? null;
    $date = ($is_post ? $_POST['date'] : $_GET['date']) ?? null; // supervision_date
    ...
    $sql_session = "SELECT 
                    ...
                    WHERE qw.t_id = ? AND qw.p_id = ? AND qw.supervision_date = ?";

    $stmt_session = $conn->prepare($sql_session);
    $stmt_session->bind_param("sss", $t_id, $p_id, $date);
    $stmt_session->execute();
    $result_session = $stmt_session->get_result();
    $session_info   = $result_session->fetch_assoc();
    $stmt_session->close();
...
// --------------------------------------------------------
// 5. ดึงคำถามจากฐานข้อมูล (ให้เหลือ 1 ข้อ / 1 display_order)
// --------------------------------------------------------
// เลือก id ที่ต่ำสุดของแต่ละ display_order เพื่อกันข้อซ้ำ
$sql_questions = "
                ...
                <?php if ($status === 1): ?>
                    ...
                    <div class="alert alert-success text-center">
                        ...
                        <div class="d-flex justify-content-center gap-2 mt-3">
                            ...
                            <?php endif; ?>
                            <!-- ปุ่มสำหรับ quickwin certificate (ถ้ามีไฟล์ทำแล้ว) -->
                        </div>
                    </div>
                <?php else: ?>
